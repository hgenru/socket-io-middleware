...
module.exports = class Context extends EventEmitter {
    ...
    sendResult(postfix, data) {
        ...
        this.emitAsync('before-result', this)
        .catch((err) => {
            let error = this.router._formatError(err);
            postfix = this.router.options.errorPostfix;
            this.result = {status: postfix, data: error};
        })
        .then(() => {
            let responseEventName = this.event.name + DELIMITER + this.result.status;
            this.socket.emit(responseEventName, this.result.data);
            this.emitAsync('result', this);
        });
    }
